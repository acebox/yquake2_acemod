...
// ace - randomizer v3x spawn points
void SP_monster_rspawnspot(edict_t *ent);

// ace - new mission pack spawns
// rogue
...
// xatrix
	...
	{"monster_chick_heat", SP_monster_chick /*SP_monster_chick_heat*/},
	{"monster_gladb", SP_monster_gladiator /*SP_monster_gladb*/},
...
// ace - rogue map entities - end

// ace - zaero/evolve map entities - start
...
// ace - zaero/evolve map entities - end
...
ED_CallSpawn(edict_t *ent)
{
	...
	// ace - testing "version 2" randomizer (hijack/hook @ spawn function level, then randomize/branch out after).
	// ace - hook here for substitutions (initial level load)
//	ace_SpawnRandomizer (ent);
...
// ace - make things interesting, no longer cap at 3.  add room for "variable" skill
	...
	// ace - set (float to int) level.skill_level.
	level.skill_level = (int)skill_level;
	...
	while (1)
	{
		...
		if (ent != g_edicts)
		{
			// ace - skill levels 6+ might break things, but should be pretty fun
			if (skill->value > 5)
			{ // skill 6+ all items/enemies/etc.. from easy -> hard+ will be allowed
				ent->spawnflags &=
					~(SPAWNFLAG_NOT_EASY | SPAWNFLAG_NOT_MEDIUM |
					SPAWNFLAG_NOT_HARD);
				if (skill->value > 6)
				{ // skill 7+ add in coop/dm/multiplayer items/enemies/etc..
					ent->spawnflags &=
		...
		// ace - this is for the monster randomizer, any monster spawned with this flag will get converted
		// to a spawn spot (spot1-4), their class/type will be tallied into the system, and yeah.
		if (!Q_strncasecmp(ent->classname, "monster_", 8))
		{
			ent->flags |= FL_RMONSTER;
		}
	...
	// ace - monster randomizer - start
	// setup for level init.
	level.rmonsterinfo.t1_init				= false;
	level.rmonsterinfo.t2_init				= false;
	level.rmonsterinfo.t3_init				= false;
	level.rmonsterinfo.t4_init				= false;

	level.rmonsterinfo.t1_spawned			= level.time;
	level.rmonsterinfo.t2_spawned			= level.time + 15;
	level.rmonsterinfo.t3_spawned			= level.time + 30;
	level.rmonsterinfo.t4_spawned			= level.time + 60;

// ace - this is now stored in level.num_wave, keep things progressive
//	level.rmonsterinfo.num_wave				= 0;
//	level.rmonsterinfo.max_wave_monsters	= 25;

	level.rmonsterinfo.init_mspawn			= true; // monster spawns should be tallied up, GO!

	level.rmonsterinfo.max_mspawns[MONSTER_INSANE] += 1 + (randk() & 4); // set random insane guys to 1-5 minimum
	// ace - monster randomizer - end


	// ace - func to spawn extra things.
	ace_AddItems();
}
...
// ace - added other startup items
	PrecacheItem(FindItem("Push"));
	...
	/*
	ace - @client.h MAX_CLIENTWEAPONMODELS = 20 
	(so technically 0-20 makes 21 weapons max, not 15?)
	@acemod.h the extras defined are as follows:
