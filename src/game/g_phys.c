...
SV_AddGravity(edict_t *ent)
{
	...
	// ace - (fully) utilize gravityVector
...
SV_Physics_Toss(edict_t *ent)
{
	...
	if ((ent->movetype != MOVETYPE_FLY) &&
		...
		(ent->movetype != MOVETYPE_WALLBOUNCE)) // xatrix
	{
		SV_AddGravity(ent);
	}
	...
	if (trace.fraction < 1)
	{
		if (ent->movetype == MOVETYPE_WALLBOUNCE) // xatrix
		{
			backoff = 2.0;
		}
		...
		{
			// ace - lets try something...
			float vlen;
			
			vlen = (VectorLength(ent->velocity))/2;
			if (vlen > 50)
				vlen = 50;
			vlen *= 0.01;
//			backoff = 1;
			backoff = 1 + (ACE_RANDOM * vlen);
		}
...
G_RunEntity(edict_t *ent)
{
	...
	switch ((int)ent->movetype)
	{
		...
		case MOVETYPE_WALLBOUNCE: // xatrix
			SV_Physics_Toss(ent);
			break;
		default:
