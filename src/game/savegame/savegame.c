...
InitGame(void)
{
	...
	/* ace - extra cvars */
	sv_allow_grapple = gi.cvar("sv_allow_grapple", "0", CVAR_SERVERINFO|CVAR_LATCH);		// ace - grappling hook is enabled server-side
	sv_allow_push = gi.cvar("sv_allow_push", "1", CVAR_SERVERINFO|CVAR_LATCH);				// ace - "offhand" push is enabled server-side
	sv_spawn_chainfist = gi.cvar("sv_spawn_chainfist", "0", CVAR_SERVERINFO|CVAR_LATCH);	// ace - allows all clients to spawn with a chainfist.
	g_infighting = gi.cvar("g_infighting", "0", CVAR_SERVERINFO);
...
//					gi.dprintf ("WriteField1: function not in list, can't save game");
//					break;
				}
...
//					gi.dprintf ("WriteField1: mmove not in list, can't save game");
//					break;
				}
