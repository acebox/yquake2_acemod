...
//			kick = 25;
		...
		if (plane)
		{
			T_Damage(other, self, self->owner, self->velocity, self->s.origin,
					plane->normal, self->dmg, 0 /* kick */, DAMAGE_ENERGY, mod);
		}
		else
		{
			T_Damage(other, self, self->owner, self->velocity, self->s.origin,
					vec3_origin, self->dmg, 0 /* kick */, DAMAGE_ENERGY, mod);
		}
	}
	else
	{
...
fire_rail
	this function has been modified to allow for "wall piercing"
	set by RAIL_MAX_WALLHITS.  Max wall thickness is set by the
	variable RAIL_MAX_WALLHITS.  Simple jumps keep things in check,
	only 1 rail trail is then drawn after everything.  Need testing
	for between wall visibility.
=================================================================
*/

#define RAIL_MAX_WALLHITS		5
#define RAIL_MAX_WALLSIZE		64
...
fire_rail(edict_t *self, vec3_t start, vec3_t aimdir, int damage, int kick)
{
	...
	int mask;
	qboolean water;
	int numwalls = 0; // ace - for wall piercing code
	qboolean pnoise_alert = false; // ace - ai helper (for multi-wall piercing)
	vec3_t	trail_start;
	...
	VectorCopy (start, trail_start);

	refire: // ace - test refiring

	// ace - after 5 walls, just hang it up and draw the trail already.
	if (numwalls > RAIL_MAX_WALLHITS)
	{
		goto end;
//		return;
	}
	...
	/* send gun puff / flash */
	if (numwalls > 0) // ace - add "puff" on exiting wall
	{
		gi.WriteByte(svc_temp_entity);
	...
	gi.WriteByte(svc_temp_entity);
	gi.WriteByte(TE_FLECHETTE); // ace - also try out TE_HEATBEAM_SPARKS
	gi.WritePosition(tr.endpos);
	gi.WriteDir(tr.plane.normal);
	gi.multicast(tr.endpos, MULTICAST_PVS);

	// ace - added special check for skyboxes
	if ((tr.surface) && (tr.surface->flags & SURF_SKY))
	{
		goto end; // nothing to see here, move along..
	}


	// ace - moved below to end:
/*
	gi.WriteByte(svc_temp_entity);
...
*/
	...
	// ace - only allow one, first one would be closest to the player.
	if (self->client && pnoise_alert == false)
	{
		pnoise_alert = true;
		PlayerNoise(self, tr.endpos, PNOISE_IMPACT);
	}

	// ace - check for piecing
	vec3_t test;

	// bail if in SKY
	VectorMA(tr.endpos, RAIL_MAX_WALLSIZE, aimdir, test);

	if (gi.pointcontents(test) & MASK_SOLID) // false alarm check
	{
		VectorMA(tr.endpos, RAIL_MAX_WALLSIZE/2, aimdir, test);

		if (gi.pointcontents(test) & MASK_SOLID) // "finer" check
		{
			VectorMA(tr.endpos, RAIL_MAX_WALLSIZE/4, aimdir, test);
		}
	}
	
	if (!(gi.pointcontents(test) & MASK_SOLID)) // empty air?
	{
		tr = gi.trace(test, NULL, NULL, start, ignore, CONTENTS_SOLID|CONTENTS_WINDOW); // trace *back* only looking for solid bsp/window
		VectorCopy (tr.endpos, start); // set new "start" for trace
		numwalls++; // hit another wall
//		if (numwalls > RAIL_MAX_WALLSIZE)
//			goto end;
//		else
		goto refire; // jump to refire
//		fire_rail(self, test, aimdir, damage, kick);
	}

	end: // ace - for "finalizing" shots

	// ace - send 1 rail effect *after* everything
	/* send gun puff / flash */
	gi.WriteByte(svc_temp_entity);
	gi.WriteByte(TE_RAILTRAIL);
	gi.WritePosition(trail_start);
	gi.WritePosition(tr.endpos);
	gi.multicast(self->s.origin, MULTICAST_PHS);
...
bfg_think(edict_t *self)
{
	...
	if (deathmatch->value)
	{
		dmg = 5; // leave deathmatch alone!!! (ace - ok, lol)
	}
	else
	{
	 // ace - singleplayer is usually @ 10 units, but... this bfg absorbs damage and deals it back (up to 5x)
//		dmg = 10;
		dmg = self->dmg;
		// cap and reset laser damage
		if (dmg < 1) // start low, it stacks up quick.  T_Damage adds damage to inflictor (bfg)
			dmg = 1;
		if (dmg > 50)
			dmg = 50;
		if (dmg != self->dmg)
