...
AI_SetSightClient(void)
{
	...
	while (1)
	{
		...
		if (check > /*game.maxclients*/ game.maxentities)
		{
			check = 1;
		}
...
FoundTarget(edict_t *self)
{
	...
	if (self->enemy->client || (self->enemy->health < self->health)) // ace - added "check" for weaker targets
	{
	...
	self->show_hostile = level.time + 1 + (skill->value*1); /* wake up other monsters */
...
// ace - basic findradius/bot-style hunting code, used for infighting/short-range hunting.
edict_t *
ace_FindTarget(edict_t *self)
{
	edict_t *target;

	target = findradius(NULL, self->s.origin, 512);

	while (target)
	{ // do a while loop of findradius
		if (target->inuse && target != self)
		{ // paranoia checks already?
			if (visible(self,target) && infront(self,target))
			{ // is it visible and infront
				if ((target->client) || 
					((target->svflags & SVF_MONSTER) && (target->health < self->health)))
				{ // is the target a player, or a weaker monster.
					if (target->health > 0)
					{ // make sure its still in the game.
						if ((strcmp(target->classname,self->classname) != 0) ||	(target->enemy == self))
						{ // verify target different class, or its already mad at me.. GO FOR IT!
							gi.dprintf("ace_FindTarget: %s found close-range target %s.\n", self->classname, target->classname);
							return target;
						}
					}
				}
		...
		target = findradius(target, self->s.origin, 512); // keep loop going
	}
 ...
 */
// ace - "modified" FindTarget for infighting below (NOW WITH MOAR CVAR!!!1!ONE)
qboolean
FindTarget(edict_t *self)
{
	...
	// ace - this check for the "global player target" (if its not in ambush)
	if ((level.sight_entity_framenum >= (level.framenum - 1)) &&
		!(self->spawnflags & 1) && level.sight_entity != NULL)
	{
	...
	else if (level.sound_entity_framenum >= (level.framenum - 1) && level.sound_entity != NULL)
	{
		...
		heardit = true;
			 ...
			 !(self->spawnflags & 1) && level.sound2_entity != NULL)
	{
	...
	else
	{
		...
		// ace - crude "infighting" here, alternate checks for players/weaker monsters.  could be costly, but its 2015..
		if (!client)
		{
//			if (self->spawnflags & 1)
//				return false; // this should keep ambush monsters "happy"
			// ace - try and find something on our own
//			client = ace_FindTarget(self);
			// ace - paranoia bail-out check.
//			if (!client)
				return false;
		}
	}
	...
	// ace - this checks if the "client" is a monster
	else if (client->svflags & SVF_MONSTER)
	{
		// ace - monster has no enemy, bail.. or do we?  special check for infighting.
		if (!client->enemy)
		{
			if (!strcmp(self->classname, client->classname)) // same classname, bail.
				return false;
//			if (client->health > self->health) // enemy has more health, don't instigate.
//				return false;
		}
		else if (client->enemy && client->enemy != self) // they are not mad at us
		{
			if (!strcmp(self->classname, client->classname) || (g_infighting->value == 0)) // same classname, get his enemy
			{
				client = client->enemy; // ace - lolparadox?
			}
		}
		else if (client->enemy->flags & FL_NOTARGET)
		{
			return false;
		}
	}
	...
	if (!heardit)
	{
		// ace - if player is in a dark area, take into account range and speed
		float spd;
		...
		spd = VectorLength (client->velocity); // ace - check if there's any movement

		/* is this an actual client and also in an spot too dark to be seen? */
		if ((client->client) && (client->light_level <= 5) &&
		(r > RANGE_NEAR) && (spd > (0 + (MAX_SKILL_LEVEL - level.skill_level))) ) // ace - also take into account distance and speed (make them somewhat dumber on lower skill levels)
		{
			return false;
		}
		...
		self->enemy = client; // this "client" is now our enemy

		// didn't hear it, anything but player noise
		if (strcmp(self->enemy->classname, "player_noise") != 0)
		{
			self->monsterinfo.aiflags &= ~AI_SOUND_TARGET;

			if (!self->enemy->client) // not a client
			{
//				// gets angry at enemy
//				self->enemy = self->enemy->enemy;

				// is our enemy, an monster?
				if (self->enemy->svflags & SVF_MONSTER)
				{
					// ace - monster has no enemy, bail.. or do we?  special check for infighting.
					if (!self->enemy->enemy)
					{
						if (!strcmp(self->classname, self->enemy->classname) || 
							(g_infighting->value == 0)) // same classname, or infighting off then bail.
						{
							return false;
						}
/*						if (self->enemy->health > self->health) // enemy has more health, don't instigate.
						{
							self->enemy = NULL;
							return false;
						}*/
					}
					else if ((self->enemy->enemy != self) && 
								(g_infighting->value == 0))
					{	// infighting off, and they aren't mad at us - join in on the fight.
						self->enemy = self->enemy->enemy;
					}
				}

				if (!self->enemy->client && (g_infighting->value == 0)) // ace - added infighting cvar check
				{
					self->enemy = NULL;
					return false;
				}
	...
	else /* heardit */
	{
		...
		if (self->spawnflags & 1) // ambush, can it be seen?
		{
			if (!visible(self, client))
			{
				return false;
