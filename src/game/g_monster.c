...
//	ent->s.renderfx &= ~(RF_SHELL_RED | RF_SHELL_GREEN | RF_SHELL_BLUE);
	...
	// ace - this flashes when monsters are in AI_BRUTAL
	if ((ent->monsterinfo.aiflags & AI_BRUTAL &&
		(level.framenum & 4)) ||
		(ent->monsterinfo.brutal_framenum > level.framenum))
	{
		ent->s.effects |= EF_COLOR_SHELL;
		ent->s.renderfx |= RF_SHELL_RED;
	}
...
monster_start(edict_t *self)
{
	...
	// ace - keep track of original size, for respawning/resurrecting (medic/etc..)
	VectorCopy(self->mins, self->omins);
	VectorCopy(self->maxs, self->omaxs);

// ace - this should probably be depreciated, as it's handled elsewhere.
#if 0
	if (st.item)
	{
	...
	else if (!self->item) // ace - added for now, as this gets called post-spawn
	{
		// ace - hijack this, randomize if nothing defined
		ace_MonsterItemRandomizer(self);
	}
#endif
