...
#define GAMEVERSION "acemod"

/* ace - for level_local array for monster randomizer */
#define MAX_MONSTERTYPES		42
...
typedef enum
{
	...
	MOVETYPE_BOUNCE,
	MOVETYPE_WALLBOUNCE, // from xatrix (ionripper)
	MOVETYPE_NEWTOSS    /* from rogue (deathball), might try on other stuff */
} movetype_t;
...
/* 
	ace_mspawninfo_t
	- this should help keep track of basic spawn info for monster spawner.
	- used in an array, should be read as: level.rmonsterinfo.mspawnspot[#].spawnflags
*/
#define ACE_MAX_MSPAWNTYPES	3
typedef struct 
{
	vec3_t	origin;
	vec3_t	angles;
...
/*
	ace_rmonsterinfo_t 
	- moved here from acemod.h
	- the "core" of the monster randomizer
	- this struct lives off level_locals_t, which gets reset every level load.  convenient.
	- the array mspawnspot (and counter num_mspawnspot), are the new array version of the spawner (free more ents, y0)
	- the other arrays num/max_monsters[MAX_MONSTERTYPES] will replace the many, many ints below.
	- that should free up all the entities used for normal monster spawns.
	- float t#_spawned, these are timers used for spawning various tiers
*/
typedef struct
{
	// ace - FIXME: eventually have these defined as arrays, and named better.  maybe.
	// ace - these are timers
	float t0_spawned;	// frequency of tier0 enemies (insane humans, barrels, misc..)
	float t1_spawned;	// frequency of tier1 enemies (weakest/small)
	float t2_spawned;	// frequency of tier2 enemies (larger)
	float t3_spawned;	// frequency of tier3 enemies (mini-boss)
	float t4_spawned;	// frequency of tier4 enemies (bosses)

	// for initial level load, spawns X enemies on the first frame pretty much
	qboolean	t1_init;
	qboolean	t2_init;
	qboolean	t3_init;
	qboolean	t4_init;
	...
	qboolean	init_mspawn; // don't do anything untill this is set true

//	struct gclient_s *client;
	
	ace_mspawninfo_t mspawnspot[MAX_EDICTS];
	int	num_mspawnspot;	// number of spawn spots

	// "wave" settings (moved to game locals)
//	int num_wave;				// what wave are we on?
	int max_wave_monsters;		// this is to keep num_monsters in check
	qboolean wave_set;			// when wave set is true, no more spawns untill all (or close) are dead.
	
	int max_monsters;			// total number of enemies in the "pool"
	int num_monsters;			// number of active enemies in the "pool"
} ace_rmonsterinfo_t;
// ace - this struct gets used in the level locals (game local.h)
...
typedef struct
{
	...
	// ace - monster randomizer stuff, for per-level randomization
// #ifdef ACE_MRANDOMIZER
	// ace - for per-level monster randomization (ace_rmonsterinfo_t defined above)
	int	skill_level;
	ace_rmonsterinfo_t	rmonsterinfo;
	// ace - "wave" settings for randomizer
	int num_wave;				// what wave are we on?
// #endif
...
//	void (*dodge)(edict_t *self, edict_t *other, float eta);
	void (*dodge)(edict_t *self, edict_t *other, float eta, trace_t *tr); // ace - rogue adds trace into equation
	...
	// ace - rogue monsterinfo additions
	...
	// ace - for visual "flash" when monsters get AI_BRUTAL flag
	float brutal_framenum;
} monsterinfo_t;
...
struct gclient_s
{
	...
	// ace - added after stock
	qboolean trap_blew_up;
	float trap_time;

	// grappling hook
	void *grapple;              /* entity of grapple */
	int grapplestate;               /* true if pulling */
	float grapplereleasetime;       /* time of grapple release */

	// techs
	float regentime;            /* regen tech */
	float techsndtime;
	float lasttechmsg;
	...
	// for sniper rifle beeping
	int			sniperFramenum;

	// for sonic cannon
	float		startFireTime;
	
	int			cg_windup; // for chaingun
	qboolean	cg_sndup;	// sound junk
	qboolean	cg_snddn;	// same
...
struct edict_s
{
	...
	// ace - added entries below
	float	dead_timestamp;		// time marker for monster/corpse death, after CORPSE_DECAY delay allow "normal" gibbing.
	
	int plat2flags;
	vec3_t offset;
	vec3_t gravityVector;			// this will be very usefull for bending gravity
	edict_t *bad_area;
	edict_t *hint_chain;
	...
	// ace - for monster randomizer v3x+
	int	monstertype;
	int	monstertier;
...
// ace - additional global-header-stuff should go here
#include "acemod.h"
#endif /* GAME_LOCAL_H */