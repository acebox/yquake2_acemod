...
//	if (damage >= self->gib_damage)
	{
		...
		for (n = 0; n < 4; n++)
		{
			ThrowGib(self, "sprites/null.sp2", damage*4, GIB_ORGANIC);
		}
		// ace - TEST: shouldn't be too taxing
		for (n = 0; n < 4; n++)
		{
			ThrowGib(self, "sprites/null.sp2", damage*4, GIB_METALLIC);
		}
...
//	self->s.origin[2] += 1;
	...
	AngleVectors(self->s.angles, forward, NULL, up);
	VectorMA(self->velocity, 300, forward, self->velocity);
	VectorMA(self->velocity, 500, up, self->velocity);
}
...
berserk_jump_wait_land(edict_t *self)
{
	...
	if (self->groundentity == NULL)
	{
		...
		VectorMA(self->velocity, 25, forward, self->velocity); // ace - this should be awesome, or look hilarious.
...
berserk_blocked(edict_t *self, float dist)
{
	...
	// ace - skill-variable jump height, berserkers are a special case of awesome lol
	maxdn = 256 + (level.skill_level*15) + (ACE_RANDOM*192); // no f*$&s given that day
	maxup = 16 + (level.skill_level*5) + (ACE_RANDOM*10);
...
SP_monster_berserk(edict_t *self)
{
	...
	self->health = 240;
	self->gib_health = -600; // -60;
//	self->gib_damage = 60; // -60;
	self->mass = 450;
	...
	if (!self->yaw_speed)
	{
		...
		if (skill->value == 2)
			self->yaw_speed = 45; // make him somewhat mobile
	...
	if (skill->value > 4)
	{
		...
		self->monsterinfo.power_armor_power = 5*(skill->value);
	}
