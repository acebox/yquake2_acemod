...
infantry_blocked(edict_t *self, float dist)
{
	...
	// ace - skill-variable jump height
	maxdn = 192 + (level.skill_level*10) + (ACE_RANDOM*128);
...
infantry_duck(edict_t *self, float eta)
{
	...
	{
		self->monsterinfo.duck_wait_time = level.time + eta + (0.1 * (7 - skill->value));
	}
...
// ace - from zaero, for the handler (infantry + hound)
