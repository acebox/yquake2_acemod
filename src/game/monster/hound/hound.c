...
qboolean hound_check_jump (edict_t *self)
{
	...
	if (distance > 100)
	{
		if (random() < (0.9 - (level.skill_level * FRAMETIME))) // ace - tweaked to factor in skill levels
			return false;
	}
...
// ace - rogue ai additions end
void
hound_jump_down(edict_t *self)
{
	vec3_t forward, up;
...
void
hound_jump_wait_land(edict_t *self)
{
	if (!self)
	{
		return;
...
mframe_t hound_frames_jump_up[] = {
	{ai_move, -8, NULL},
	{ai_move, -8, NULL},
	{ai_move, -8, NULL},
	{ai_move, -8, hound_jump_up},
	{ai_move, 0, NULL},
	{ai_move, 0, hound_jump_wait_land},
	{ai_move, 0, NULL}
};

mmove_t hound_move_jump_up = {
	FRAME_runStart, 
	FRAME_runEnd,
   	hound_frames_jump_up,
   	hound_run
};

mframe_t hound_frames_jump_down[] = {
	{ai_move, 0, NULL},
	{ai_move, 0, NULL},
	{ai_move, 0, hound_jump_down},
	{ai_move, 0, NULL},
	{ai_move, 0, NULL},
	{ai_move, 0, hound_jump_wait_land},
	{ai_move, 0, NULL}
...
mmove_t hound_move_jump_down = {
	FRAME_runStart, 
	FRAME_runEnd,
   	hound_frames_jump_down,
   	hound_run
};

void
hound_tryjump(edict_t *self)
{
	if (!self)
	{
		return;
	...
	{
		self->monsterinfo.currentmove = &hound_move_jump_up;
	}
	else
	{
		self->monsterinfo.currentmove = &hound_move_jump_down;
	}
}

qboolean
hound_blocked(edict_t *self, float dist)
{
	if (!self)
	{
		return false;
	...
	maxdn = 192 + (level.skill_level*10) + (ACE_RANDOM*45); // 256;
	maxup = 33 + (level.skill_level*5) + (ACE_RANDOM*25); // 68;
	...
	// ace - only let the dogs use the platforms on nightmare+
	if ((blocked_checkplat(self, dist)) && (level.skill_level > 2))
	{
		return true;
	}
...
void SP_monster_hound (edict_t *self)
{
	...
	self->yaw_speed = 30 + (skill->value * 5);

	// ace - reworked these values, a "dog" shouldn't be as strong as the strogg parasite..lol
//	self->health = 175; // ace - 175
	self->health = 15 + (skill->value * 5); // range 15 to 50
	self->gib_health = -50;
	self->mass = 100; // 250 seems heavy
	...
	// ace - rogue ai additions
	self->monsterinfo.blocked = hound_blocked;
	// ace - rogue ai end
...
void hound_createHound(edict_t *self, float healthPercent)
{
	...
	// ace - reworked these values, a "dog" shouldn't be as strong as the strogg parasite..lol
	hound->yaw_speed = 30 + (skill->value * 5);
	hound->health = healthPercent * (15 + (skill->value * 5));
	// range 15 to 50, factoring healthPercent
	hound->mass = 100; // 250 seems heavy
	...
	// ace - rogue ai additions
	hound->monsterinfo.blocked = hound_blocked;
	// ace - rogue ai end
