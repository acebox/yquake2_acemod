...
void soldierh_fire(edict_t *self, int);
void soldierh_blind(edict_t *self);
...
soldier_attack(edict_t *self)
{
	...
	if ((!(self->monsterinfo.aiflags & (AI_BLOCKED | AI_STAND_GROUND))) &&
		...
		((r < (skill->value * 0.11)) && // ace - changed from .25 to .11 for new max skill
		 (self->s.skinnum <= 3)))
	{
...
soldier_jump_now(edict_t *self)
{
	...
	// ace - TEST - nevermind how they forgot the groundent NULL and origin + 1, but the Z velocity might not be too 
...
void
soldier_jump2_now(edict_t *self)
{
	vec3_t forward, up;
...
void
soldier_jump_wait_land(edict_t *self)
{
	if (!self)
	{
		return;
...
soldier_blocked(edict_t *self, float dist)
{
	...
	// ace - skill-variable jump height
	float maxup, maxdn;

	maxdn = 128 + (level.skill_level*10) + (ACE_RANDOM*64); // 256;
	maxup = 16 + (level.skill_level*5); // 68;

//	if (blocked_checkjump(self, dist, 192, 40))
	if (blocked_checkjump(self, dist, maxdn, maxup))
	{
		soldier_jump(self);
		return true;
	}
		...
		vec3_t point /* unused */)
{
	...
	if (self->health <= self->gib_health)
	{
		...
		for (n = 0; n < 4; n++)
		{
			ThrowGib(self, "models/objects/gibs/sm_meat/tris.md2", damage*(1+ACE_RANDOM), GIB_ORGANIC);
		}
		...
		if (r < 0.33)
		{
			ThrowHead(self, "models/objects/gibs/head/tris.md2", damage, GIB_ORGANIC);
		}
		else if (r < 0.66)
		{
		...
		{
			ThrowHead(self, "models/objects/gibs/skull/tris.md2", damage, GIB_ORGANIC);
		}
		// ace - MOAR GIBS - end
...
soldier_duck(edict_t *self, float eta)
{
	...
	if ((self->monsterinfo.currentmove == &soldier_move_jump) ||
		(self->monsterinfo.currentmove == &soldier_move_jump2))
	{
		return;
	}
...
SP_monster_soldier_light(edict_t *self)
{
	...
	self->health = 20;
	self->gib_health = -200;
//	self->gib_damage = 20;		// ace - for gibbing
...
SP_monster_soldier(edict_t *self)
{
	...
	self->health = 30;
	self->gib_health = -200;
//	self->gib_damage = 30;		// ace - for gibbing
}
...
SP_monster_soldier_ss(edict_t *self)
{
	...
	self->health = 40;
	self->gib_health = -200;
//	self->gib_damage = 25;		// ace - for gibbing
}

// ace - xatrix mission pack soldiers
...
mframe_t soldierh_frames_attack6[] = {
	...
	{ai_run, 12, NULL},
	{ai_run, 12, NULL},
	{ai_run, 17, soldierh_attack6_refire}
};
...
void
soldierh_dodge(edict_t *self, edict_t *attacker, float eta, trace_t *tr)
{
	float r;
...
void
soldierh_sidestep(edict_t *self)
{
	if (!self)
	{
		return;
	...
	if (self->s.skinnum <= 3)
	{
		if (self->monsterinfo.currentmove != &soldierh_move_attack6)
		{
			self->monsterinfo.currentmove = &soldierh_move_attack6;
		}
	}
	...
	{
		if (self->monsterinfo.currentmove != &soldierh_move_start_run)
		{
			self->monsterinfo.currentmove = &soldierh_move_start_run;
		}
	}
...
void
soldierh_duck(edict_t *self, float eta)
{
	float r;
	...
	if (r > (skill->value * 0.14)) // ace - changed from .3 to .14 to accomidate new skills
	{
		self->monsterinfo.nextframe = FRAME_duck01;
		self->monsterinfo.currentmove = &soldierh_move_duck; // ace - was 3-skill
		self->monsterinfo.duck_wait_time = level.time + eta + (0.1 * (MAX_SKILL_LEVEL - skill->value));
	}
	else
	{
		self->monsterinfo.nextframe = FRAME_attak301;
...
/* mframe_t soldierh_frames_jump2[] = {
	{ai_move, -8, NULL},
	{ai_move, -4, NULL},
	{ai_move, -4, NULL},
	{ai_move, 0, soldier_jump_now},
...
soldierh_jump(edict_t *self)
{
	...
	if (self->enemy->s.origin[2] > self->s.origin[2])
	{
		self->monsterinfo.currentmove = &soldierh_move_jump2;
	}
	else
	{
...
qboolean
soldierh_blocked(edict_t *self, float dist)
{
	// ace - skill-variable jump height
	float maxup, maxdn;
	...
	// ace - skill-variable jump height
	maxdn = 192 + (level.skill_level*10) + (ACE_RANDOM*64);
	maxup = 16 + (level.skill_level*5);
...
mframe_t soldierh_frames_blind[] = {
	{ai_move, 0, soldier_idle},
	{ai_move, 0, NULL},
	{ai_move, 0, NULL},
	{ai_move, 0, NULL},
...
mmove_t soldierh_move_blind = {
	FRAME_stand101,
   	FRAME_stand130,
   	soldierh_frames_blind,
   	soldierh_blind
};

void
soldierh_blind(edict_t *self)
{
	if (!self)
	{
		return;
	...
	self->monsterinfo.currentmove = &soldierh_move_blind;
}
...
SP_monster_soldier_h(edict_t *self)
{
	...
	self->monsterinfo.blocked = soldierh_blocked;
	self->monsterinfo.duck = soldierh_duck;
	self->monsterinfo.unduck = monster_duck_up;
	self->monsterinfo.sidestep = soldierh_sidestep;

	if (self->spawnflags & 8) /* blind */
	{
		self->monsterinfo.stand = soldierh_blind;
	}
...
SP_monster_soldier_ripper(edict_t *self)
{
	...
	self->health = 50;
	self->gib_health = -200;
//	self->gib_damage = 50;		// ace - for gibbing
}
