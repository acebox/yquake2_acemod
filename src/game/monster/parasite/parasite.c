...
parasite_blocked(edict_t *self, float dist)
{
	...
	// ace - skill-variable jump height
	maxdn = 256+ (level.skill_level*15) + (ACE_RANDOM*72);
	maxup = 33 + (level.skill_level*5);
...
// ace - they're only stroggified (smart) enough to work platforms on hard+
	if (blocked_checkplat(self, dist) && (level.skill_level > 1))
	{
		return true;
	}
