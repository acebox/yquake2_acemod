...
// ace - make this interesting (based on BossExplode)
void
TankExplode(edict_t *self)
{
	vec3_t org;
	int n;
	...
	switch (self->count++)
	{
		...
		case 1:
			...
			for (n = 0; n < 2; n++)
			{
				ThrowGib(self, "sprites/null.sp2", 750, GIB_METALLIC);
			}
			break;
		case 2:
			org[0] += 24;
		...
		case 3:
			...
			for (n = 0; n < 2; n++)
			{
				ThrowGib(self, "sprites/null.sp2", 750, GIB_ORGANIC);
			}
			break;
		case 4:
			org[0] -= 48;
	...
	self->nextthink = level.time + 0.1 + (random() * 0.3);
}
...
*/
	...
	{
		/* check for gib */
		// ace - gave tanks a "boss" explosion
//		if (damage >= self->gib_health)
		if (self->health <= self->gib_health)
		{
			gi.sound(self, CHAN_VOICE, gi.soundindex("misc/udeath.wav"), 1, ATTN_NORM, 0);
...
/*	if (self->flags & FL_RMONSTER)
	...
	{
		self->health = 1000;
		self->gib_health = -700; // -225;
		// ace - lets make things interesting.
		if (!self->yaw_speed)
		{
			self->yaw_speed = 40; // default
			if (skill->value == 3)
				self->yaw_speed = 75; // make him somewhat mobile
			else if (skill->value > 3)
				self->yaw_speed = 95;  // ace - IIRC, this should be "bot-like"
		}
		if (skill->value >= 2)
		{
			...
			self->monsterinfo.power_armor_power = 15*(skill->value);
		}
	...
	{
		self->health = 750;
		self->gib_health = 150; // -200;
		// ace - lets make things interesting.
		if (!self->yaw_speed)
		{
			self->yaw_speed = 25; // default
			if (skill->value == 3)
				self->yaw_speed = 50; // make him somewhat mobile
			else if (skill->value > 3)
