...
MakronRailgun(edict_t *self)
{
	...
	// ace - be more accurate on higher difficulties (railgun fires through walls, hehehehe)
	if (skill->value >= 1)
	{
...
/*	if (self->flags & FL_RMONSTER)
	...
	self->health = 3000 + (skill->value * 75);
	self->gib_health = -2000;
	self->mass = 750;

	self->flags |= FL_IMMUNE_LASER;
	self->flags |= FL_NO_KNOCKBACK; // ace - this should work with bosses

	// ace - lets make things interesting.
	if (!self->yaw_speed)
	{
		self->yaw_speed = 60; // default
		if (skill->value == 3)
			self->yaw_speed = 80; // make him somewhat mobile
