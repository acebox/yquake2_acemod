...
//		if (ent->owner) // shouldn't this check if this is owned by me?
//		{
//			continue;
//		}
...
// ace - made it a definite less than, b/c looks like it can get hung up on going between enemies with the same health while checking for equals.
...
medic_idle(edict_t *self)
{
	...
	if (ent)
	{
		...
		self->monsterinfo.aiflags |= AI_MEDIC; // switch over to medic ai
		FoundTarget(self);
	}
}
...
void
medic_dodge(edict_t *self, edict_t *attacker, float eta, trace_t *tr)
{
	if (random() > 0.5) // 0.25)
	{
		return;
...
medic_cable_attack(edict_t *self)
{
	...
	// ace - tweaked resurrection distance by skill
	max_dist = 256; // Vanilla Medic
	if (skill->value == 3)
		max_dist = 384; // Nightmare Medic
	else if (skill->value >= 3)
		max_dist = 512; // YGBKM Medic
	...
	else if (self->s.frame == FRAME_attack50)
	{
		...
		self->enemy->monsterinfo.aiflags = AI_DO_NOT_COUNT; // ace - keep some things in check here
		self->enemy->target = NULL;
		self->enemy->targetname = NULL;
		self->enemy->combattarget = NULL;
		self->enemy->deathtarget = NULL;
		self->enemy->owner = self;
// ace - this wouldn't work too well with the randomizer, so do some things manually. START
		ED_CallSpawn(self->enemy);
/*
		self->enemy->deadflag = DEAD_NO;					// resurrected
		self->enemy->health = self->enemy->max_health;		// rejuvenated
		VectorCopy(self->enemy->omins, self->enemy->mins);	// restore size (mins)
		VectorCopy(self->enemy->omaxs, self->enemy->maxs);	// restore size (maxs)
		gi.linkentity(self->enemy);

//		self->enemy->monsterinfo.currentmove = self->enemy->monsterinfo.stand;
		self->enemy->monsterinfo.stand(self->enemy);

		if (self->enemy->flags & FL_FLY)
			flymonster_start (self->enemy);		
		else if (self->enemy->flags & FL_SWIM)
			swimmonster_start (self->enemy);		
		else
			walkmonster_start (self->enemy);		 */
// ace - END - fixed things.
		...
		// ace - definitely reset enemy if they were mad at me, peace things up!
		if (self->enemy->enemy == self)
			self->enemy->enemy = NULL; // should just give him somebody, but hey..
		// ace - you now fight fow me!
		if (self->oldenemy /*&& self->oldenemy->client*/ )
		{
			self->enemy->enemy = self->oldenemy;
			FoundTarget(self->enemy);
			if (skill->value > 1) // ace - this should be fun on hard+
				ai_charge(self->enemy, 0); // face target
		}
	}
	else
	{
...
SP_monster_medic(edict_t *self)
{
	...
	self->health = 300;
	self->gib_health = 130; // -130;
	self->mass = 800;

	// ace - lets make things interesting.
	if (!self->yaw_speed)
	{
		self->yaw_speed = 30; // default
		if (skill->value == 3)
			self->yaw_speed = 60; // make him somewhat mobile
		else if (skill->value > 3)
			self->yaw_speed = 90;  // ace - IIRC, this should be "bot-like"
	}
	...
	if (skill->value >= 2)
	{
		...
		self->monsterinfo.power_armor_power = 25*(skill->value);
	}
