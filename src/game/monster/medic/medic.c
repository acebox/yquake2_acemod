...
//	self->s.origin[2] += 1;
	...
	VectorMA(self->velocity, 350, forward, self->velocity);
	VectorMA(self->velocity, 450, up, self->velocity);
}

void
medic_jump2_now(edict_t *self)
{
	vec3_t forward, up;
...
//	self->s.origin[2] += 1;
	...
	AngleVectors(self->s.angles, forward, NULL, up);
	VectorMA(self->velocity, 450, forward, self->velocity);
	VectorMA(self->velocity, 550, up, self->velocity);
}

void
medic_jump_wait_land(edict_t *self)
{
	if (!self)
	{
		return;
	...
	if (self->groundentity == NULL)
	{
		...
		VectorMA(self->velocity, 250, forward, self->velocity); // ace - this should be awesome, or look hilarious.
...
medic_jump(edict_t *self)
{
	...
	{
		self->monsterinfo.currentmove = &medic_move_jump2;
	}
	else
	{
		self->monsterinfo.currentmove = &medic_move_jump;
	}
}
...
medic_blocked(edict_t *self, float dist)
{
	...
	if (blocked_checkjump(self, dist, 768, 128))
	{
		medic_jump(self);
		return true;
	}
...
//		self->gib_health = -130;
		self->gib_health = -600;
		self->mass = 600;
		self->yaw_speed = 60 + (5 * (skill->value));
		MedicCommanderCache();
	}
	else
	{
...
//		self->gib_health = -130;
		self->gib_health = -600;
		self->mass = 400;
		self->yaw_speed = 40 + (5 * (skill->value));
	}
	...
	if (self->mass > 400)
	{
		...
		{
			self->monsterinfo.monster_slots = 10;
		}
