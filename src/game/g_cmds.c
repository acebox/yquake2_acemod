...
//		msg = "noclip ON\n";
		msg = "Smashing Pumpkins Into Small Piles Of Putrid Debris.\n";
	}
...
Cmd_WeapPrev_f(edict_t *ent)
{
	...
	for (i = 1; i <= MAX_ITEMS; i++)
	{
		...
		// ace - skip over push, it breaks weapon switching.
		if (it == FindItem("Push"))
		{
			continue;
		}
...
/*
 * "Ace Special" commands. Mainly debugging stuff for the mod, might have some usefull things.
 */
//level.rmonsterinfo.num_wave
void
Cmd_Ace_f(edict_t *ent)
{
	char *name;
	...
	{
		gi.cprintf( ent, PRINT_HIGH,
				"This debug command is only available as a cheat.\n");
//				"You must run the server with '+set cheats 1' to enable this command.\n");
		return;
	}
	...
	if (Q_stricmp(name, "ltime") == 0)
	{
		gi.dprintf ("ace_dprint: level.time is %f \n", level.time);
	}
	else if (Q_stricmp(name, "waveinfo") == 0)
	{
		gi.dprintf ("ace_dprint: stats\nlevel.time = %f\nnum_wave = %i\nmax_wave_monsters = %i\n(t1 spawn timer) %f\n(t0 spawn timer) %f\n", level.time, level.num_wave, level.rmonsterinfo.max_wave_monsters, level.rmonsterinfo.t1_spawned - level.time, level.rmonsterinfo.t0_spawned - level.time);
	}
	else if (Q_stricmp(name, "minit") == 0)
	{
		gi.dprintf ("ace_minit: Monster Randomizer Spawn Initialization, GO!\n");
		level.rmonsterinfo.wave_set = false;
		level.rmonsterinfo.t1_init = false;
		level.rmonsterinfo.t1_spawned = level.time + FRAMETIME;
	}
	else if (Q_stricmp (name, "mreset") == 0)
	{
		gi.dprintf ("ace_mreset: Monster Randomizer Time Reset, GO!\n");
		level.rmonsterinfo.wave_set = false;
		level.rmonsterinfo.t1_spawned = level.time + FRAMETIME;
	}
	else if (Q_stricmp (name, "mwaveup") == 0)
	{
		level.num_wave++;
		gi.dprintf ("ace_mreset: monster randomizer wave increased to %i\n", level.num_wave);
	}
	else if (Q_stricmp (name, "mwavedn") == 0)
	{
		level.num_wave--;
		if (level.num_wave<0)
		{
			level.num_wave = 0; // ace - no level -1 here
		}
		gi.dprintf ("ace_mreset: monster randomizer wave decreased to %i\n", level.num_wave);
	}
/*	else if (Q_stricmp (cmd, "mspawn") == 0)
	{
		if (gi.argc() == 3)
		{
			mpick = (int)strtol(gi.argv(2), (char **)NULL, 10);
		}
	} */
...
ClientCommand(edict_t *ent)
{
	...
	else if (Q_stricmp(cmd, "notarget") == 0 || Q_stricmp(cmd, "fhshh") == 0)
	{
		Cmd_Notarget_f(ent);
	}
	else if (Q_stricmp(cmd, "noclip") == 0 || Q_stricmp(cmd, "idspispopd") == 0)
	{
		Cmd_Noclip_f(ent);
	}
