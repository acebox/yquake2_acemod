...
SP_info_player_deathmatch(edict_t *self)
{
	...
	if (!deathmatch->value)
	{
		/* ace - convert unused spawns into random monster spawns on nightmare+ */
		if (skill->value > 2)
		{
			self->classname = "misc_insane";
			self->monstertype = MONSTER_INSANE;
...
SP_info_player_coop(edict_t *self)
{
	...
	if ((Q_stricmp(level.mapname, "jail2") == 0) ||
		...
		(Q_stricmp(level.mapname, "xware") == 0))
	{
		/* invoke one of our gross, ugly, disgusting hacks */
		...
		if (attacker == self)
		{
			switch (mod)
			{
				...
				default:
					...
					if (IsNeutral(self) || !self->client) // ace - test.
					{
...
TossClientWeapon(edict_t *self)
{
	...
	// ace - exclude these
	if (item && (strcmp(item->pickup_name, "Push") == 0))
	{
		item = NULL;
	}
		...
		int damage, vec3_t point /* unused */)
{
	...
	if (!self->deadflag)
	{
		...
		PlayerResetGrapple(self); // ace - grapple
...
InitClientPersistant(gclient_t *client)
{
	...
	// ace - add some startup goodness
	if (sv_spawn_chainfist->value)
	{
		item = FindItem("Chainfist");
		client->pers.inventory[ITEM_INDEX(item)] = 1;
		if (sv_spawn_chainfist->value == 2)
		{
			client->pers.weapon = item;
		}
	}
	if (sv_allow_grapple->value)
	{
		item = FindItem("Grapple");
		client->pers.inventory[ITEM_INDEX(item)] = 1;
	}
	if (sv_allow_push->value)
	{
		item = FindItem("Push");
		client->pers.inventory[ITEM_INDEX(item)] = 1;
	}
	...
	client->pers.max_bullets = MAX_BULLETS_NORM;
	client->pers.max_shells = MAX_SHELLS_NORM;
	client->pers.max_rockets = MAX_ROCKETS_NORM;
	client->pers.max_grenades = MAX_GRENADES_NORM;
	client->pers.max_cells = MAX_CELLS_NORM;
	client->pers.max_slugs = MAX_SLUGS_NORM;

// ace - extra ammo
	client->pers.max_magslug = MAX_MAGSLUG_NORM;
	client->pers.max_trap = MAX_TRAP_NORM;
	client->pers.max_prox = MAX_PROX_NORM;
	client->pers.max_tesla = MAX_TESLA_NORM;
	client->pers.max_flechettes = MAX_FLECHETTES_NORM;
// ace - extra ammo
...
ClientBegin(edict_t *ent)
{
	...
	else
	{
		...
		if (game.maxclients > 1)
		{
			...
			// ace - scale enemy health up for each player connected (eventually, also damage)
			if (coop->value)
			{
				ace_EnemyScaleUp();
			}
...
ClientDisconnect(edict_t *ent)
{
	...
	// ace - scale down enemy health when a player drops the server
	if (coop->value)
	{
		ace_EnemyScaleDn();
	}
