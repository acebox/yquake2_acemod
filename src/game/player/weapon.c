...
Pickup_Weapon(edict_t *ent, edict_t *other)
{
	...
	// ace - add something here to always pickup blasters and grappling hooks (so they won't spam levels)
	if (ent->item == FindItem("blaster") || ent->item == FindItem("grapple"))
	{
		ent->flags &= ~FL_RESPAWN; // just incase
		return true;
	}
		...
		int *fire_frames, void (*fire)(edict_t *ent))
{
	...
	if (ent->client->weaponstate == WEAPON_ACTIVATING)
	{
		...
		{
			if (Q_stricmp(ent->client->pers.weapon->pickup_name, "Chaingun") == 0)
			{
				ent->client->cg_windup = 0;
			}
	...
	if (ent->client->weaponstate == WEAPON_FIRING)
	{
		for (n = 0; fire_frames[n]; n++)
		{
			if (ent->client->ps.gunframe == fire_frames[n])
			{
				...
				if (Q_stricmp(ent->client->pers.weapon->pickup_name, "Chaingun") == 0)
				{
					int windup_goal = CHAINGUN_WINDUP;

					if (ApplyHaste(ent))
						windup_goal /= 2;

					if (ent->client->cg_windup < windup_goal)
					{
						continue;
					}
				}

				if (!ApplyStrengthSound(ent))
				{
		...
		if (Q_stricmp(ent->client->pers.weapon->pickup_name, "Chaingun") == 0)
		{
				  ...
				  ent->client->buttons) & BUTTON_ATTACK))
			{
				...
				ent->client->cg_windup++;

				if (ent->client->cg_windup < 1)
				{
					ent->client->cg_sndup = false;
					ent->client->cg_snddn = true;
				}
				
				if (ent->client->cg_sndup == false)
				{
					ent->client->cg_sndup = true;
					gi.sound(ent, CHAN_AUTO, gi.soundindex("weapons/chngnu1a.wav"), 1, ATTN_IDLE, 0);
				}
				ent->client->weapon_sound = gi.soundindex("weapons/chngnl1a.wav");

				if (ent->client->cg_windup > windup_goal)
					ent->client->cg_windup = windup_goal;
			}
			else
			{
//				ent->client->cg_windup--;
				ent->client->cg_windup -= 2;

				if (ent->client->cg_windup < 1)
				{
					ent->client->cg_snddn = false;
					ent->client->cg_sndup = true;
				}
				
				if (ent->client->cg_snddn == false)
				{
					ent->client->cg_snddn = true;
...
Chaingun_Fire(edict_t *ent)
{
	...
	// ace - changed sound, use CHAN_WEAPON
	float vol = 1.0;
	if (is_silenced)
	{
		vol = 0.2;
	}
	gi.sound(ent, CHAN_WEAPON, gi.soundindex("daedalus/daedatck1.wav"), vol, ATTN_NORM, 0);
...
weapon_supershotgun_fire(edict_t *ent)
{
	...
	fire_shotgun(ent, start, forward, damage, kick,
			DEFAULT_SHOTGUN_HSPREAD*2, DEFAULT_SHOTGUN_VSPREAD*2,
