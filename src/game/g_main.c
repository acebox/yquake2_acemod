...
// ace - extra cvars
cvar_t *sv_allow_grapple;	// ace - grappling hook is enabled server-side
cvar_t *sv_allow_push;		// ace - "offhand" push is enabled server-side
cvar_t *sv_spawn_chainfist;	// ace - should be a cheat, but allows all clients to spawn with a chainfist.
cvar_t *g_infighting;		// toggles monster infighting
...
G_RunFrame(void)
{
	...
	// ace - monster randomizer v3x+, hook here and see if we need to add any monsters.
	ace_MonsterRandomizerGo();
	// ace - should add another function, to despawn certain enemies if they are too far from the player.
