...
/*	if(EMPNukeCheck(self, start))
	...
	case 1:
	default:
		fire_bullet(self, start, forward, AC_BULLET_DMG, AC_BULLET_KICK, DEFAULT_BULLET_HSPREAD, DEFAULT_BULLET_VSPREAD, MOD_TARGET_LASER /*MOD_AUTOCANNON*/);
...
void SP_monster_autocannon(edict_t *self)
{
	...
	if (!self->onFloor)
		...
		base->movetype = MOVETYPE_TOSS; // ace - FIXME: add the correct movetype eventually, for now testing...
