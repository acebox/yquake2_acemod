...
// ace - defined MAX_GIBS_PER_FRAME in acemod.h, q2 default is 20.
int	gibsthisframe = 0;
int lastgibframe = 0;
// ace - seperated gibs from debris, because.
int debristhisframe = 0;
int lastdebrisframe = 0;
...
VelocityForDamage(int damage, vec3_t v)
{
	v[0] = 150.0 * crandom();
	v[1] = 150.0 * crandom();
	v[2] = 50.0 + 150.0 * random();

	// ace - NOTE: can do bonus here for velocity+
	if (damage < 50)
	{ // ace - try some "maths" here
		VectorScale(v, 0.7+(random()*(damage/100)), v);
	}
	else
	{
		VectorScale(v, 1.2, v);
...
// ace - definte MIN/MAX velocities
float MIN_GIBVEL = -600;
float MAX_GIBVEL = 600;
float MIN_GIBVELZ = 250; /* always some upwards */
float MAX_GIBVELZ = 999;
...
ClipGibVelocity(edict_t *ent)
{
	...
	if (ent->velocity[0] < MIN_GIBVEL)
	{
		ent->velocity[0] = MIN_GIBVEL;
	}
	else if (ent->velocity[0] > MAX_GIBVEL)
	{
		ent->velocity[0] = MAX_GIBVEL;
	}

	if (ent->velocity[1] < MIN_GIBVEL)
	{
		ent->velocity[1] = MIN_GIBVEL;
	}
	else if (ent->velocity[1] > MAX_GIBVEL)
	{
		ent->velocity[1] = MAX_GIBVEL;
	}

	if (ent->velocity[2] < MIN_GIBVELZ)
	{
		ent->velocity[2] = MIN_GIBVELZ;
	}
	else if (ent->velocity[2] > MAX_GIBVELZ)
	{
		ent->velocity[2] = MAX_GIBVELZ;
	}
}

/* ===================================================== */

// ace - proto for gib_touch below
...
gib_think(edict_t *self)
{
	...
	if (self->s.frame == 10)
	{
		SpawnDamage(TE_BLOOD, self->s.origin, vec3_origin);
		self->think = G_FreeEdict;
//		self->nextthink = level.time + 8 + random() * 10;
		self->nextthink = level.time + 2 + random() * 5;
	}
}
...
gib_touch(edict_t *self, edict_t *other /* unused */, cplane_t *plane, csurface_t *surf /* unused */)
{
	...
	if (!plane)
		...
		SpawnDamage(TE_BLOOD, self->s.origin, plane->normal);
	...
	if (plane)
	{
		...
		SpawnDamage(TE_MOREBLOOD, self->s.origin, plane->normal);
		...
		SpawnDamage(TE_MOREBLOOD, self->s.origin, vec3_origin);
}
...
ThrowGib(edict_t *self, char *gibname, int damage, int type)
{
	...
	if (gibsthisframe > MAX_GIBS_PER_FRAME)
	{
		return;
	}
...
ThrowHead(edict_t *self, char *gibname, int damage, int type)
{
	...
	if (self->client) // ace - fallback for the "just incase" - this should not happen
	{
...
//	self->s.effects &= ~EF_FLIES;
	self->s.effects &= ~(EF_COLOR_SHELL | EF_POWERSCREEN | EF_DOUBLE | EF_QUAD | EF_PENT | EF_FLIES);
...
//	self->nextthink = level.time + 10 + random() * 10;
	self->nextthink = level.time + 7 + random() * 5;
...
ThrowClientHead(edict_t *self, int damage)
{
	...
	{
		self->s.skinnum = 0;
		ThrowHead(self, gibname, damage, GIB_ORGANIC);
		return;
	}
...
ThrowDebris(edict_t *self, char *modelname, float speed, vec3_t origin)
{
	...
	if (level.framenum > lastdebrisframe)
	{
		debristhisframe = 0;
		lastdebrisframe = level.framenum;
	}

	debristhisframe++;

	if (debristhisframe > MAX_DEBRIS_PER_FRAME)
	{
		return;
	}
...
SP_func_explosive(edict_t *self)
{
	...
	// ace - added option of spawning in deathmatch 2
	if (deathmatch->value && (deathmatch->value != 2))
	{
...
misc_easterchick_think(edict_t *self)
{
	...
	{
		if (!(self->s.effects & EF_FLIES) && (random() < 0.33))
		{
			self->s.effects |= EF_FLIES;
			self->s.sound = gi.soundindex("infantry/inflies1.wav");
		}
		if ((self->s.effects & EF_FLIES) && (random() < 0.25))
		{
			self->s.effects &= ~EF_FLIES;
			self->s.sound = 0;
		}
		...
		csurface_t *surf /* unused */)
{
	...
	// ace - open porters up to these movetypes
	...
	// ace - moved client-specific stuff here
	if (other->client) 
	{
	...
	else
	{
		...
		for (i = 0; i < 3; i++)
		{
//			other->s.angles[i] = ANGLE2SHORT(dest->s.angles[i] - other->s.angles[i]);
			other->s.angles[i] = dest->s.angles[i];
		}
		if ((other->svflags & SVF_MONSTER) && (other->enemy))
		{
			other->show_hostile = level.time + 2 + (skill->value*2);
			/* wake up other monsters, since we just TELEPORTED! */
		}
